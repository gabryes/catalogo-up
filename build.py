...
def css_base() -> str:
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .comercial-destaque h2 {{ color: #333; margin-bottom: 0.5rem; font-size: 1.4rem; }}
    .comercial-destaque p {{ color: #666; margin-bottom: 1.2rem; }}
    .btn-comercial {{
    ...
    .card a.ver {{ margin-top: 0.9rem; color: {COBERTA_AZUL}; text-decoration: none; font-weight: 600; font-size: 0.9rem; }}
    ...
    .detalhe .info-item {{ border-top: 1px solid #eef2f7; padding-top: 0.6rem; margin-top: 0.6rem; }}
    .detalhe .info-item .rotulo {{ font-size: 0.78rem; text-transform: uppercase; color: #8a98b3; }}
    ...
    .social-whatsapp svg {{ width: 24px; height: 24px; display: block; }}
...
def cabecalho_html(titulo: str, prefix: str, subtitulo: str = "") -> str:
    # Evitando f-strings aninhadas complexas para prevenir erros de renderização
    sub_tag = ""
    if subtitulo:
        sub_tag = "<p>" + escapa_html(subtitulo) + "</p>"
    else:
        sub_tag = "<p>Serviços da comunidade universitária</p>"
    ...
    html.append('<title>' + escapa_html(titulo) + ' | Catálogo UP</title>')
    html.append('<style>' + css_base() + '</style>')
    html.append('</head>')
    html.append('<body>')
    html.append('<header>')
    html.append('  <h1><a href="' + prefix + 'index.html">Catálogo UP</a></h1>')
    html.append('  ' + sub_tag)
    html.append('</header>')
    return "\n".join(html)
...
def gerar_home(servicos, categorias):
    ...
    dados_json = json.dumps([
        {"id": s["id"], "nome": s.get("nome", ""), "categoria": s.get("categoria", ""), "descricao": s.get("descricao", "")}
        for s in servicos
    ], ensure_ascii=False)

    opcoes_cat = "\n".join(f'<option value="{escapa_html(c)}">{escapa_html(c)}</option>' for c in categorias)
...
<div class="vazio" id="sem-resultados" style="display:none; text-align:center; padding:2rem; color:#8a98b3;">
...
def gerar_lista_servicos(servicos, categorias):
    ...
    html += """
<main>
<a class="voltar" href="../index.html">&larr; Voltar ao início</a>
<div class="barra-busca">
...
def gerar_detalhe_servico(servico):
    ...
    for rotulo, valor in campos:
        if valor and str(valor).strip():
            ...
            linhas.append(f'<div class="info-item"><div class="rotulo">{rotulo}</div><div class="valor">{v}</div></div>')

    icones_contato = []
    whatsapp = servico.get("whatsapp", "").strip()
    if whatsapp:
        numero_limpo = re.sub(r'[^0-9]', '', whatsapp)
        icones_contato.append(f'''<a href="https://wa.me/{numero_limpo}" target="_blank" rel="noopener" class="social-whatsapp" aria-label="WhatsApp"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"><path d="M12.01 2.01c-5.52 0-10 4.48-10 10 0 1.75.46 3.4 1.26 4.83L2 22l5.3-.1.13.07c1.33.71 2.85 1.1 4.44 1.1 5.52 0 10-4.48 10-10s-4.48-10-10-10zm0 18.39c-1.57 0-3.1-.42-4.43-1.21l-.32-.19-3.14.06.06-3.07-.21-.33c-.87-1.38-1.33-2.98-1.33-4.63 0-4.63 3.77-8.4 8.4-8.4s8.4 3.77 8.4 8.4-3.77 8.4-8.4 8.4zm4.75-6.37c-.26-.13-1.54-.76-1.78-.85-.24-.09-.41-.13-.58.13-.17.26-.66.85-.81 1.02-.15.17-.31.19-.57.06-.26-.13-1.1-.4-2.1-1.29-.77-.69-1.29-1.54-1.44-1.8-.15-.26-.02-.4.11-.53.12-.12.26-.31.39-.46.13-.15.17-.26.26-.43.09-.17.04-.32-.02-.45-.06-.13-.58-1.39-.79-1.91-.21-.51-.42-.44-.58-.45-.15-.01-.32-.01-.49-.01-.17 0-.45.06-.68.31-.23.25-.89.87-.89 2.12 0 1.25.91 2.46 1.03 2.63.13.17 1.79 2.73 4.33 3.83.6.26 1.08.42 1.44.54.61.19 1.16.16 1.6.1.49-.07 1.54-.63 1.76-1.24.22-.61.22-1.13.15-1.24-.07-.11-.26-.17-.52-.3z"/></svg></a>''')

    instagram = servico.get("instagram", "").strip()
    if instagram:
        user = instagram.lstrip('@')
        icones_contato.append(f'''<a href="https://instagram.com/{escapa_html(user)}" target="_blank" rel="noopener" class="social-instagram" aria-label="Instagram"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg></a>''')
...
def obter_caminho_credenciais() -> str:
    ...
    raise FileNotFoundError("Credenciais do Google não encontradas.")
...
def ler_planilha():
    ...
    if not sheet_key:
        raise EnvironmentError("GOOGLE_SHEETS_KEY não definida.")
    ...
    for linha in registros:
        ...
        if not linha_norm.get("nome"):
            continue
        servico = {col: str(linha_norm.get(col, "") or "").strip() for col in COLUNAS_ESPERADAS}
        servicos.append(servico)

    return garantir_unicos_slugs(servicos)
...
def gerar_site(servicos):
    ...
    categorias = sorted({s.get("categoria", "") for s in servicos if s.get("categoria", "")})
    
    escrever_arquivo(PUBLIC_DIR / "index.html", gerar_home(servicos, categorias))
    escrever_arquivo(PUBLIC_DIR / "services" / "index.html", gerar_lista_servicos(servicos, categorias))

    for s in servicos:
        dir_servico = PUBLIC_DIR / "services" / s["id"]
        escrever_arquivo(dir_servico / "index.html", gerar_detalhe_servico(s))

    print("[Catálogo UP] Site gerado com sucesso.")
